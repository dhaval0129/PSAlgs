 ...
 * Hint: 
 * 
 * 1] Track prev_zero and prev_prev_zero and calculate max = curr - prev_prev_zero
 *
 */
public class LongestContinuousSeq {
    ...
    private static int maxOnesIndex(int arr[], int n) {
        ...
        for (int curr = 0; curr < n; curr++) {
            // If current element is 0, then calculate the difference between
            // curr and prev_prev_zero
            if (arr[curr] == 0) {
                // Update result if count of 1s around prev_zero is more
                if (prev_prev_zero != - 1 && curr - prev_prev_zero > max_count) {
